...
class DHS(nn.Module):
    def __init__(self):
        ...
        self.kmer = kmerModel(in_channels=23).apply(weights_init_uniform)
    ...
    def forward(self, input):
        ...
        feature = self.bilstm2(feature)[0]
        feature = feature[:,:,:64]+feature[:,:,64:]
        ...
        batchsize = kmer.shape[0]

        # stackFeature=torch.stack([rednaEmbed,refeature,rekmer],dim=-2).view(batchsize,3,80,60)
        # stackFeature=torch.stack([rednaEmbed,rekmer],dim=-2).view(batchsize,2,80,60)
        stackFeature=torch.stack([dnaEmbed,feature],dim=1).view(batchsize,2,80,60)

        out_stack3 = self.resSeNet(stackFeature)

        # combine3 = torch.cat([dnaEmbed,feature,kmer],dim=-1)
        # out_combine3 = self.mlp3(combine3)

        out_kmer = self.mlp(self.flatten(kmer))
        out_feature = self.mlp(self.flatten(feature))
        out_embed = self.mlp(self.flatten(dnaEmbed))

        return out_stack3,(out_kmer,out_embed)
...
dic = {'A': 1, 'B': 22, 'U': 23, 'J': 24, 'Z': 25, 'O': 26, 'C': 2, 'D': 3, 'E': 4, 'F': 5, 'G': 6, 'H': 7, 'I': 8,
       'K': 9, 'L': 10, 'M': 11, 'N': 12, 'P': 13, 'Q': 14, 'R': 15, 'S': 16, 'T': 17, 'V': 18, 'W': 19, 'Y': 20,'X': 21}
