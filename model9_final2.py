...
class DHS(nn.Module):
    def __init__(self):
        ...
        self.attn_fe = CrossMoHAttention1(64,8,shared_head=4,routed_head=4,attn_drop=0.2,proj_drop=0.2,qkv_bias=True,qk_norm=True,head_dim=8)
        self.attn_ef = CrossMoHAttention1(64,8,shared_head=4,routed_head=4,attn_drop=0.2,proj_drop=0.2,qkv_bias=True,qk_norm=True,head_dim=8)
        ...
        #                         )
        self.moet = SelectAttention(64,8,True)
    ...
    def forward(self, input,label=None,epoch=0):
        dna_embedding = input[0]
        simple = self.cnn_simple(dna_embedding.int())
        # if input[0].shape[0] == 128*3 and epoch==15:
            # stne(simple.clone().detach().cpu().numpy(), label.clone().detach().cpu().numpy(),name='stne_1')
        dna_multicode = input[2]
        complex = self.cnn_complex(dna_multicode)
        # if input[0].shape[0] == 128*3  and epoch==15:
            # stne(complex.clone().detach().cpu().numpy(), label.clone().detach().cpu().numpy(),name='stne_2')
        eh, eq = self.rnn1(simple)
        ehide = eh[:, :, :64] + eh[:, :, 64:]
        fh, fq = self.rnn2(complex)
        fhide = fh[:, :, :64] + fh[:, :, 64:]
        # if input[0].shape[0] == 128*3  and epoch==15:
            # stne(ehide.clone().detach().cpu().numpy(), label.clone().detach().cpu().numpy(), name='stne_3')
            # stne(fhide.clone().detach().cpu().numpy(), label.clone().detach().cpu().numpy(), name='stne_4')
        # 交叉注意力
        ef = self.attn_ef(fhide, ehide)
        fe = self.attn_fe(ehide, fhide)
        # if input[0].shape[0] == 128*3  and epoch==15:
            # stne(ef.clone().detach().cpu().numpy(), label.clone().detach().cpu().numpy(), name='stne_5')
            # stne(fe.clone().detach().cpu().numpy(), label.clone().detach().cpu().numpy(), name='stne_6')
        # 残差自注意力
        ...
        # if input[0].shape[0] == 128*3  and epoch==15:
        #     stne(xe.clone().detach().cpu().numpy(), label.clone().detach().cpu().numpy(), name='stne_7')
        #     stne(xf.clone().detach().cpu().numpy(), label.clone().detach().cpu().numpy(), name='stne_8')
        out_xef = self.classifier1(self.flatten(self.moe(xe, xf)))
        # out_xef = self.moe(self.flatten(xe), self.flatten(xf))
    ...
    def predict(self, X, batch_size=128,label=None,epoch=0):
        # 设置模型为评估模式
        self.eval()
        predictions = []
        X1 = X[0].to("cuda")
        ...
        with torch.no_grad():
            for i in range(0, len(X1), batch_size):
                ...
                output, output2 = self.forward((batch1, batch2, batch3,label,epoch))
...
class MASE(nn.Module):
    ...
    def forward(self, xc, x):
        ...
        mask = self.sigmoid((avg_mask + max_mask) / 2)

        # output = x * mask + self.IN(xc.permute(0, 2, 1)).permute(0, 2, 1) * (1 - mask)  # x和xc换位
        output = self.IN(x.permute(0, 2, 1)).permute(0, 2, 1) * mask + xc * (1 - mask)  # x和xc换位
...
class MixtureOfExperts(nn.Module):
    def __init__(self, input_dim1=4800, input_dim2=4800, num_experts=4, expert_dim=256, output_dim=1, k=2):
        ...
        #     nn.Linear(input_dim1, 100),
        #     nn.ReLU(),
        #     nn.Linear(100, num_experts),
        # )
        self.gating1 = nn.Linear(input_dim2, num_experts)
