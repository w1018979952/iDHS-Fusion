...
class DHS(nn.Module):
    ...
    def forward(self, input):
        ...
        xe = self.mase1(ehide, self.attn1(ehide))
        xf = self.mase2(fhide, self.attn2(fhide))

        # out_xef =self.fc(self.resSeNet(torch.stack([xe,xf],dim=1)))
        out_xef = self.mlp1(self.flatten(xe+xf))
