...
class DHS(nn.Module):
    ...
    def forward(self, input):
        ...
        eh, eq = self.rnn1(dnaEmbed)
        ehide = eh[:, :, :64] + eh[:, :, 64:]

        fh, fq = self.rnn2(feature)
        fhide = fh[:, :, :64] + fh[:, :, 64:]
        ...
        # 加性自注意力
        res_e = self.attn1(ehide)
        res_f = self.attn2(fhide)
        ...
        out_xef = self.classifier1(self.flatten(self.mse_ef(xe,xf)))
...
#         MASE 模块前向传播
#         输入:
#             xc (Tensor): 输入张量 1，形状为 (batch_size, seq_len, dim)
#             x (Tensor): 输入张量 2，形状为 (batch_size, seq_len, dim)
#         输出:
#             output (Tensor): 融合后的输出，形状为 (batch_size, seq_len, dim)
#         """
#         xc_tmp = xc.permute(0, 2, 1)
#
#         avg_pooled = self.avg_pool(xc_tmp).permute(0, 2, 1)
