...
class cnn_all(nn.Module):
    def __init__(self):
        ...
        self.emb = nn.Embedding(27, 13)
        self.conv1 = nn.Conv1d(in_channels=13 + 13, out_channels=64, kernel_size=5, padding='same')
        self.pool1 = nn.MaxPool1d(kernel_size=2, stride=2, padding=0)
...
class DHS(nn.Module):
    def __init__(self):
        ...
        self.cnn_all = cnn_all().apply(weights_init_uniform)
    ...
    def forward(self, input):
        ...
        x =  self.cnn_all(dna_embedding.int(),dna_multicode)
        x, eq = self.rnn1(x)
        x = x[:, :, :64] + x[:, :, 64:]
        ...
        out_xef = self.classifier1(self.flatten(x))

        part_xe = self.flatten(x)
        part_xf = self.flatten(x)
