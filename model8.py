...
class cnn(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv1d(in_channels=21, out_channels=128, kernel_size=5, padding='same')
...
class DHS(nn.Module):
    def __init__(self):
        ...
        self.attn_ef = CustomAttention().apply(weights_init_uniform)

        self.mase1 = MASE(dim=64).apply(weights_init_uniform)
        self.mase2 = MASE(dim=64).apply(weights_init_uniform)
    ...
    def forward(self, input):
        ...
        ehide = eh[:, :, :64] + eh[:, :, 64:]
        # eseq = torch.cat((eq[0, :, :], eq[1, :, :]), dim=1)--009
        ...
        # 交叉注意力
        ef = self.attn_ef(fhide, self.attn1(ehide))
        fe = self.attn_fe(ehide, self.attn2(fhide))
        ...
        # out_xef = self.classifier1(dp_xef),

        dp_xe = self.pooling_layer2(ehide)
        dp_xf = self.pooling_layer3(fhide)
        out_xe, out_xf = self.classifier2(dp_xe), self.classifier3(dp_xf),
...
class MASE(nn.Module):
    ...
    def forward(self, xc, x):
        ...
        output = x * mask + self.IN(xc) * (1 - mask)
        # output = self.IN(xc) * mask + x * (1-mask)
